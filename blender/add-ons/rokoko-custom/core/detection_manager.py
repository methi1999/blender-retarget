...
def create_internal_bone_list():
    """
    bone_list has key as bone name and common names for only left bone as values e.g. leftHand as key and leftHand, leftWrist,
    etc. as values in a list. left is replaced with right for all symmetric parts
    """
    new_bone_list = {}
...
def combine_lists(internal_list, custom_list):
    """
    Append custom_list dict to internal list (custom list may include names such as mixamo:spine)
    @param internal_list: original dict
    @param custom_list: dict to add
    @return: combined
    """
    combined_list = {}
...
def standardize_bone_name(name):
    """
    Crucial function which deals with variations in naming schemes by changing names
    @param name:
    @return:
    """
...
def detect_retarget_bones():
    """
    Maps each source armature bone to exactly one target armature bone
    @return: dict with key = source bone, val = (target_bone_name, bone_source e.g. chest)
    """
    bone_list_animated = []
    retargeting_dict = {}
