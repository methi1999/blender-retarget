...
def main_func(args):
    ...
    # ========= Run tracking_old ========= #
    bbox_scale = 1.1
    mpt_path = os.path.join(output_path, 'tracking.pickle')
    if os.path.exists(mpt_path):
        print("Loading MPT data from pickle")
        with open(mpt_path, 'rb') as f:
        ...
        with open(mpt_path, 'wb') as f:
            pickle.dump(tracking_results, f)

    # remove tracklets if num_frames is less than MIN_NUM_FRAMES
    for person_id in list(tracking_results.keys()):
        if tracking_results[person_id]['frames'].shape[0] < MIN_NUM_FRAMES:
            print("Deleting person due to insufficient frames")
    ...
    for person_id in tqdm(list(tracking_results.keys())):
        ...
        if args.run_smplify and args.tracking_method == 'pose':
            ...
            print('[WARNING] You need to enable pose tracking_old to run Temporal SMPLify algorithm!')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tracking_method', type=str, default='bbox', choices=['bbox', 'pose'],
                        help='tracking_old method to calculate the tracklet of a subject from the input video')

    parser.add_argument('--detector', type=str, default='yolo', choices=['yolo', 'maskrcnn'],
                        help='object detector to be used for bbox tracking_old')

    parser.add_argument('--yolo_img_size', type=int, default=416,
                        help='input image size for yolo detector')

    parser.add_argument('--tracker_batch_size', type=int, default=12,
                        help='batch size of object detector used for bbox tracking_old')

    parser.add_argument('--staf_dir', type=str, default='/home/mkocabas/developments/openposetrack',
                        help='path to directory STAF pose tracking_old method installed.')
    ...
    parser.add_argument('--run_smplify', action='store_true',
                        help='run smplify for refining the results, you need pose tracking_old to enable it')
