...
joints2d (n_frames, 21, 3)  # 2D keypoint detections by STAF if pose tracking_old enabled otherwise None
bboxes (n_frames, 4)        # bbox detections (cx,cy,w,h)
frame_ids (n_frames,)       # frame ids in which subject with tracking_old id #1 appears
